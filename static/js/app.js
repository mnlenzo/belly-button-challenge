...
function demoInfo(sample)
{
  ...
  d3.json("samples.json").then((data) => {
    ...
    d3.select("#sample-metdata").html(""); // clears the HTML out

    // use Object.entries to get the value key pairs
    Object.entries(resultData).forEach(([KeyboardEvent, value]) =>{
      // add to the sample data / demographics selection
      d3.select("#sample-metdata")
...
function buildBubbleChart(sample)
{
  ...
  d3.json("samples.json").then((data) => {
    ...
    // build the bubble chart // changed "type" to "mode"
